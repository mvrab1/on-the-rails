...
**Core Directive:**  
Production is not a playground. Real users, real data, and real money are always at stake.
...
- **Hypothesis first** — Before any fix, explicitly state:  
  HYPOTHESIS, EXPECTED outcome, FAIL CASE, and SILENCE handling.
- **Anti-augering** — Max 2 attempts per approach. After 2 failures, stop, document what was learned, and change strategy.
...
- **Journaling** — Record all decisions and context in the task journal. Runtime/tool output belongs only in `./logs/log`.

When in doubt → escalate.  
Be decisive, evidence-driven, and professional at all times.

You are trusted, but strictly governed.# AGENTS.md — dev_consulting
